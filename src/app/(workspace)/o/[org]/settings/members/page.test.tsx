...
// The registry has no route for this yet, so the page has to cope with its
// absence rather than assume a list.
vi.mock("@/lib/registry/registry-api", () => ({
  listMembers: async () => null,
}));

import OrganisationMembersPage from "@/app/(workspace)/o/[org]/settings/members/page";
...
describe("organisation members page", () => {
  beforeEach(() => {
    authMock.mockReset();
  ...
  it("renders the organisation named by the path", async () => {
    ...
    render(await OrganisationMembersPage(params(ULID)));

    expect(loadWorkspaceMock).toHaveBeenCalledWith(ULID);
    expect(
      screen.getByRole("heading", { level: 1, name: "Members" }),
    ).toBeDefined();
    // The registry cannot list members, and the page says so rather than
    // reporting that nobody is here.
  ...
  it("is a 404 for an organisation this account is not in", async () => {
    ...
    await expect(
      OrganisationMembersPage(params("01BX5ZZKBKACTAV9WEVGEMMVRZ")),
  ...
  it("renders nothing while the layout shows the outage", async () => {
    ...
    expect(await OrganisationMembersPage(params(ULID))).toBeNull();
  });
});
