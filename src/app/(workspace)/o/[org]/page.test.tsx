...
import OrganisationHomePage from "@/app/(workspace)/o/[org]/page";
...
describe("organisation home page", () => {
  beforeEach(() => {
    authMock.mockReset();
  ...
  it("renders the organisation named by the path", async () => {
    ...
    render(await OrganisationHomePage(params(ULID)));

    // The ULID reached the loader, which is what scopes the request.
    expect(loadWorkspaceMock).toHaveBeenCalledWith(ULID);
    expect(
      screen.getByRole("heading", { name: "Example Holdings Ltd" }),
    ).toBeDefined();
    expect(screen.getAllByText("Example Holdings Ltd").length).toBeGreaterThan(
      0,
    );
  ...
  it("is a 404 for an organisation this account is not in", async () => {
    ...
    await expect(
      OrganisationHomePage(params("01BX5ZZKBKACTAV9WEVGEMMVRZ")),
  ...
  it("renders nothing while the layout shows the outage", async () => {
    ...
    expect(await OrganisationHomePage(params(ULID))).toBeNull();
  });
});
