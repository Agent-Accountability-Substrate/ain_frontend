 ...
 * commands. The unread state is in the trigger's accessible name and not only
 * in the dot, which is `aria-hidden` — otherwise there is no way to learn an
 * unread notification exists without opening the panel.
 */
export function NotificationsMenu({
  ...
  // Derived from the record rather than the address, so the bell agrees with
  // the status pill beside it on every screen.
  const unread = assuranceStatus === "not_started" ? 1 : 0;
